...
def crawl_fundamental_daily(begin_date, end_date):
    ...
    for code in codes:
        try:
            df_fundamental = pro.daily_basic(
                ts_code=code, start_date=begin_date, end_date=end_date,
            ...
            update_requests = []
            for index in df_fundamental.index:
                document = dict(df_fundamental.loc[index])
            ...
            print(
                'Error occurs when update stock fundamental from %s to %s, %s-%s, %s at position input: %s' %
    ...
    _log.seek(0, 0)
    _log.write(
        'Update stock fundamental from %s to %s, %s-%s, %s, inserted: %4d, modified: %4d \n \n' %
...
if __name__ == '__main__':
    crawl_fundamental_daily('20190306', '20190306')
