...
_collection_name_ = 'Index_Quotation_Daily'
...
def crawl_index_daily(begin_date, end_date):
    ...
    for code in codes:
        try:
            df_quotation = ts.pro_bar(
                pro_api=api, ts_code=code, start_date=begin_date, end_date=end_date, asset='I', freq="D"
            )
            ...
            print(
                'Error occurs when update index quotation from %s to %s, %s-%s, %s at position input: %s' %
    ...
    _log.seek(0, 0)
    _log.write(
        'Update index quotation from %s to %s, %s-%s, %s, inserted: %4d, modified: %4d \n \n' %
...
if __name__ == '__main__':
    crawl_index_daily('20190307', '20190307')
