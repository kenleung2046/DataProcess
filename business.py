...
def crawl_business(end_date):
    ...
    for code in codes:
        try:
            df_bz = pro.fina_mainbz(ts_code=code, type='P')

            update_requests = []
            for index in df_bz.index:
                document = dict(df_bz.loc[index])

                update_requests.append(
                    UpdateOne(
                        {'ts_code': code,
                         'end_date': document['end_date'],
                         'bz_item': document['bz_item']},
                        {'$set': document},
                        upsert=True
                    )
                )
            ...
            # 网上调取数据次数控制
            request_api_times += 1
            if request_api_times % 60 == 0:
                print('crawl data requests api times control')
                time.sleep(70)

        except:
            print(
                'Error occurs when update stock business at %s, %s-%s, %s at position input: %s' %
    ...
    _log.seek(0, 0)
    _log.write(
        'Update stock business at %s, %s-%s, %s, inserted: %4d, modified: %4d \n \n' %
